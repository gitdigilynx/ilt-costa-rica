 ...
 */
class TotalPassengers extends FilterType
{
    use FilterTypeTrait;
    ...
    public function filter(QueryBuilder $queryBuilder, FormInterface $form, array $metadata)
    {
        ...
        if (ComparisonType::BETWEEN === $data['comparison']) {
            ...
            $queryBuilder->andWhere(sprintf('(entity.adultCount + entity.childCount) BETWEEN :%s and :%s', $paramName1, $paramName2))
                ->setParameter($paramName1, $data['value'])
                ->setParameter($paramName2, $data['value2']);
        } else {
            $paramName = static::createAlias($property);
            $queryBuilder->andWhere(sprintf('(entity.adultCount + entity.childCount) %s :%s', $data['comparison'], $paramName))
