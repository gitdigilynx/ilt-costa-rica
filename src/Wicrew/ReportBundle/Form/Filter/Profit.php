 ...
 */
class Profit extends FilterType
{
    use FilterTypeTrait;
    ...
    public function filter(QueryBuilder $queryBuilder, FormInterface $form, array $metadata)
    {
        ...
        if (!in_array('entity_supplier', $aliases))  $queryBuilder->join('entity_order.supplier', 'entity_supplier');
        if (ComparisonType::BETWEEN === $data['comparison']) {
            $paramName1 = static::createAlias($property);
            $paramName2 = static::createAlias($property);
            $queryBuilder->andWhere(sprintf('( entity.subtotalRack - entity.subtotalNet - (entity.subtotalRack * (IFNULL(entity_supplier.commission, 0) / 100))) BETWEEN :%s and :%s', $paramName1, $paramName2))
                ->setParameter($paramName1, $data['value'])
                ->setParameter($paramName2, $data['value2']);
        } else {
            $paramName = static::createAlias($property);
            $queryBuilder->andWhere(sprintf('( entity.subtotalRack - entity.subtotalNet - (entity.subtotalRack * (IFNULL(entity_supplier.commission, 0) / 100))) %s :%s', $data['comparison'], $paramName))
