 ...
 */
class Commission extends FilterType
{
    use FilterTypeTrait;
    ...
    public function filter(QueryBuilder $queryBuilder, FormInterface $form, array $metadata)
    {
        ...
        if (ComparisonType::BETWEEN === $data['comparison']) {
            ...
            $queryBuilder->andWhere(sprintf('(entity.subtotalRack * (entity_supplier.commission / 100)) BETWEEN :%s and :%s', $paramName1, $paramName2))
                ->setParameter($paramName1, $data['value'])
                ->setParameter($paramName2, $data['value2']);
        } else {
            $paramName = static::createAlias($property);
            $queryBuilder->andWhere(sprintf('(entity.subtotalRack * (entity_supplier.commission / 100)) %s :%s', $data['comparison'], $paramName))
